...
# errF <- learnErrors(filtFs, multithread=cores, randomize = TRUE);errR <- learnErrors(filtRs, multithread=cores, randomize = TRUE)
...
merged_reads <- readRDS("data/merged_reads.RDS")
...
for(sample in sample.names) {
  ...
  print(paste("Sample",sample,n,"of",number_samples,"(",(round(n/number_samples,4)*100),"%) processed.", sep = " "));n = n + 1
}

seqtab <- makeSequenceTable(merged_reads)
seqtab <- removeBimeraDenovo(seqtab, method="consensus", multithread=cores)

tax <- assignTaxonomy(seqtab, "~/Desktop/da2_test/database/current_Bacteria_unaligned.fa.gz", multithread=cores)
